...
<?php 
	...
	if(is_array($positions)){
		foreach($positions AS $k => $v){
			...
			echo "<p>{$v['title']} [<a id='descr{$v['id']}'>view description</a>] <span style='margin-left: 20px;font-size: 0.8em;'> Active=<b>".$act."</b>, Last update: {$v['timestamp']}, By: <b>{$v['pt_username']}</b></span> [<a id='position{$v['id']}'>edit</a>]</p>";
			echo "</div>";
			...
			<script>
				...
				$("#descr{$v['id']}").click(function(){
					...
					$.ajax({
			  			type: "POST",
			  			url: "ajax.php",
			  			data: { action: "position_descr", id: "{$v['id']}", type: "view"  }
					}).done(function( result ) {
			    		$("#container_p{$v['id']}").html(result);
			  		});
			  	});

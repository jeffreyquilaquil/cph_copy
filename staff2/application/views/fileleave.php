...
<?
}else if($this->uri->segment(2)=='offset'){
?>
	<h2>OFFSETTING / CHANGE OF WORK SCHEDULE FORM</h2>
	<hr/>
	<?
	if($submitted){
			echo '<p class="errortext">Your offset leave has been submitted.</p>
			<script>parent.location.reload();</script>';
	}else{
	?>
	<p>The Offsetting / Change of Work Schedule Form is used by full-time regular staff in accordance with company policy. Work can be rendered outside of the regular office hours (except Sundays) to offset absences or undertimes incurred due to emergency or health reasons, if approved by the Director or Chief Business Development Officer. The benefit of offsetting of work schedule can only be availed for a maximum of two (2) times in a month and must be completed within seven (7) business days from the time that the absence/undertime is incurred. Only employees with zero (0) time off credits can render work outside of the regular office hours to offset absences or undertimes. For detailed information regarding the administration of paid time off, refer to the Code of conduct and/or the Employee Handbook. For information on requesting a leave of absence contact the Human Resources Department.</p>
<?php
	$num = 0;
	...
	?>
		<?php if(file_exists($signature)){ clearstatcache(); ?>
		<tr>
			<td>Signature</td>
			<td><img src="<?= $this->config->base_url().$signature ?>"/><br/><a href="<?= $this->config->base_url().'upsignature/'.str_replace('/','-',$_SERVER['REQUEST_URI']).'/' ?>">Click Here to Change your Signature</a></td>
		</tr>
	<?php } ?>
	<tr>
	...
	<form action="" method="POST" onSubmit="displaypleasewait();" enctype="multipart/form-data">
		<tr>
			<td width="30%">Please specify reason of absence<br/>
				<i style="color:#555;"></i>
			</td>
		...
		<tr>
			...
			<td><input id="leaveStart" name="leaveStart" type="text" class="forminput datetimepick" placeholder="Month day, year hour:min" value="<?= ((isset($_POST['leaveStart']))?$_POST['leaveStart']:'') ?>"></td>
		</tr>
		<tr>
			<td>End Date and Time of Absence</td>
			<td><input id="leaveEnd" name="leaveEnd" type="text" class="forminput datetimepick" placeholder="Month day, year hour:min" value="<?= ((isset($_POST['leaveEnd']))?$_POST['leaveEnd']:'') ?>"></td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td>
				...
				<input type="submit" value="Submit Leave" class="padding5px"/>
			</td>
		</tr>
	</form>
	</table>
...
<?php }else{ ?>
	<h2>PAID TIME OFF(PTO) / LEAVE REQUEST FORM</h2>
	<hr/>
	<?
	if($submitted){
	...
	<?php	
		if(isset($numLeaves) && count($numLeaves)>=5){ echo '<p class="errortext">Please note that this is your '.$this->staffM->ordinal(count($numLeaves)).' leave application for the month of '.date('F').'.</p>'; }
		if(!empty($errortxt)) echo '<p class="errortext"><b>Please check your inputted values:</b><br/>'.$errortxt.'</p><hr/>';
	?>
	...
	<form action="" method="POST" onSubmit="displaypleasewait();" enctype="multipart/form-data">
		...
		<tr>
			<td><br/></td>
			<td>
				<input type="hidden" name="submitType" value="leave"/>
				<input type="submit" value="Submit Leave" class="padding5px"/>
			</td>
		</tr>
	</form>

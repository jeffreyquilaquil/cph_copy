...
$result = $db->selectQuery("applicants a", "a.id,a.lname,a.fname,a.mname,a.email,
	IF(isNew = 0, p.title, (SELECT title FROM newPositions WHERE a.position=posID LIMIT 1)) as position,
	a.date_created,a_s.status,a.uploads,a.timestamp,a.text_resume",$filter." ORDER BY a.date_created DESC","LEFT JOIN positions p ON a.position=p.id LEFT JOIN applicant_status a_s ON a_s.id = a.status");

...
if($_POST['action'] == "position_descr"){
	...
	echo "<p>{$v['title']} [<a id='descr{$v['id']}'>".$text." description</a>] <span style='margin-left: 20px;font-size: 0.8em;'> Active=<b>".$act."</b>, Last update: {$v['timestamp']}, By: <b>{$v['pt_username']}</b></span> [<a id='position{$v['id']}'>edit</a>]</p>";
	if($_POST['type']== 'view')
	...
	echo <<<EOF
			<script>
				...
				$("#descr{$v['id']}").click(function(){
					$loader
					$.ajax({
			  			type: "POST",
			  			url: "ajax.php",
			  			data: { action: "position_descr", id: "{$v['id']}", type: "{$text}" }
					}).done(function( result ) {
			    		$("#container_p{$v['id']}").html(result);
			  		});
			  	});
...
}else if($_POST['action'] == "position_edit"){

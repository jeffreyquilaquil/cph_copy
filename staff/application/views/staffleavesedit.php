...
<?php 	
	if($row->dateApproved!='0000-00-00'){
		...
		if(($row->iscancelled==0 || $row->iscancelled==4)&& ($this->user->accessFull==true || $this->staffM->checkStaffUnderMeByID($row->empID_fk)==true)){
...
<?php if($row->dateApproved=='0000-00-00' && ($row->iscancelled==0 || $row->iscancelled==4)){ 
	...
	}else if(($row->iscancelled==0 || $row->iscancelled==4) && $this->user->accessFullHR==true){
?>
...
<?php
	...
	if($row->iscancelled==3 && $this->user->accessFullHR==true){

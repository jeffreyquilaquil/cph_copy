<div class="incident_info"> 
	...
	<form name="hr_incident" method="post" entype="multipart/form-data" action="">
		<h2>HR Incident Number <?php echo $ticket->cs_post_id; ?></h2>
		<table class="tableInfo">
			...
			<tr>
				...
				<td>
					<?php echo '<div class="'. str_replace(' ', '', strtolower($ticket->cs_post_urgency) ).'">'.$ticket->cs_post_urgency.'</div>'; ?>
				</td>
			</tr>
			<?php
				...
				<td>
					<?php 
						...
						foreach ($urgency_labels as $key => $value) {
							...
							echo '<input id="'.$labels.'" type="radio" name="urgency" value="'.$value.'" /> <label for="'.$labels.'" class="'.$labels.'">'.$value.'</label>';
						}
					?>
				</td>
			</tr>		
		...
		foreach ($conversations as $key => $conversation) {
			...
			if( $conversation->cs_msg_type == 2 AND $this->user->empID == $ticket->cs_post_empID_fk ){
				...
				echo '<div class="'.$div_class.'">';
					echo 'Message from: '. $all_staff[ $conversation->reply_empUser ]->name;
...
<form id="custom_ans_form">
	...
	<?php }elseif($this->access->accessFull == true && ($this->uri->segment(4) == 'resolved' ||  $this->uri->segment(4) == 'cinc')){?>

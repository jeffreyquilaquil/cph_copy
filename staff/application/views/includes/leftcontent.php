...
if($this->user!=false && count($row)>0){
	...
	echo '<ul id="leftMenu">';
		...
		if($content=='staffinfo' && ($this->accessFullHR==true && $this->user->username != $row->username || $this->user->level>0)){
		...
		if($content!='staffinfo' && ($this->accessFullHR==true || $this->user->level>0)){

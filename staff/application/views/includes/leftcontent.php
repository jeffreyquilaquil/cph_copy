...
if($this->user!=false && count($row)>0){
	...
	echo '<ul id="leftMenu">';
		...
		if($content=='staffinfo' && (count(array_intersect($this->myaccess, array('full', 'hr')))>0 && $this->user->username != $row->username || $this->user->level>0)){
		...
		if($content!='staffinfo' && (count(array_intersect($this->myaccess, array('full', 'hr')))>0 || $this->user->level>0)){

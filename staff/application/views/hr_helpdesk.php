	...
	<table id="new" class="datatable">
		<thead>
			...
			<?php foreach ($ActiveIncident as $active_key => $active_val) { ?>
				<tr>
					<td class="td_hover">
	      			<!--<a href="<?php echo $this->config->base_url(); ?>hr_cs/HrIncident/<?php echo $active_val->cs_post_id; ?>" class="iframe" data-balloon-length="large" data-balloon="Detail of incident" data-balloon-pos="right"> </a>-->
	      			<?php echo "$active_val->cs_post_id";?>	
	      			</td>
					...
					<td><a href="<?php echo $this->config->base_url(); ?>hr_cs/give_update" class="iframe">Give Update</a></td>					
				</tr>
			<?php } ?>     
	</table>

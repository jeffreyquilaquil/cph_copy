			...
			if(count($data['row']) > 0){				
				if(isset($_POST) && !empty($_POST)){					
					if($_POST['submitType']=='pdetails' || $_POST['submitType']=='jdetails' || $_POST['submitType']=='cdetails'){
						...
						if(count($what2update) >0){
							if($this->access->accessFullHR==false){
									...
									foreach($what2update AS $k=>$val):
										...
										$upNote .= $this->txtM->defineField($k).' from <i>'.$o.'</i> to <u>'.$this->staffM->infoTextVal($k, $val).'</u><br/>';			
									endforeach;
						...
						$this->staffM->addMyNotif($data['row']->empID, 'Cancelled update field request:<br/>'.$this->txtM->defineField($_POST['fld']).' - '.$this->staffM->infoTextVal($_POST['fld'], $_POST['fname']), 5);
				...
				$data['perfTrackRecords'] = $this->staffM->getQueryResults('staffCoaching', 'coachID, coachedDate, coachedEval, status, selfRating, supervisorsRating, finalRating,	dateSupAcknowledged, date2ndMacknowledged, dateEmpAcknowledge, (SELECT CONCAT(fname," ",lname) AS n FROM staffs WHERE empID=coachedBy LIMIT 1) AS coachedByName, dateGenerated', 'empID_fk="'.$data['row']->empID.'"','', 'dateGenerated DESC');
		...
		if($this->access->accessFullHR==false){
			...
			if(isset($_POST) && !empty($_POST)){				
				if($_POST['submitType']=='Update'){
					...
					$ntext .= $this->txtM->defineField($_POST['fieldN']).' - ';
					...
					$ntext = $this->user->name.' disapproved your personal details update request: '.$this->txtM->defineField($_POST['fieldN']).' - '.$_POST['fieldV'].'<br/>Reason: '.$_POST['notes'];
	...
	public function coachingform(){
		...
		if(isset($_POST) && !empty($_POST)){
			if($_POST['submitType']=='acknowledged'){
				...
				$in = $this->staffM->getSingleInfo('staffCoaching', 'coachID, empID_fk, selfRating, supervisorsRating, supervisor', 'coachID="'.$id.'"', 'LEFT JOIN staffs ON empID=empID_fk');
				
				if(count($in) && $in->supervisor!=0 && $in->selfRating!='' && $in->supervisorsRating!=''){
					$this->staffM->addMyNotif($in->supervisor, 'Coaching performance evaluation for employee and supervisor are already done. You may now evaluate scores and input recommendations. Click <a href="'.$this->config->base_url().'coachingform/evaluate/'.$id.'/" class="iframe">here</a> to view details.', 0, 1, 0);
				}
				...
				if($_POST['recommendation']==7) $recText = $_POST['otherRemarks'];
				else $recText = $recArr[$_POST['recommendation']];
				
				
				
				//send notification to employee, supervisor and 2nd level manager to acknowledge evaluation result
				$snemp = $this->staffM->getSingleInfo('staffCoaching', 'coachID, empID_fk, supervisor, (SELECT supervisor FROM staffs s WHERE s.empID=staffs.supervisor LIMIT 1) AS supSupervisor', 'coachID="'.$id.'"', 'LEFT JOIN staffs ON empID=empID_fk');
				if(count($snemp)>0){
					$snote = 'Done with coaching evaluation. Final rating is "'.$recText.'". Click <a href="'.$this->config->base_url().'coachingform/acknowledgment/'.$id.'/" class="iframe">here</a> to view details and acknowledge.';
					$this->staffM->addMyNotif($snemp->empID_fk, $snote, 0, 1, 0); //to employee
					$this->staffM->addMyNotif($snemp->supervisor, $snote, 0, 1, 0); //to supervisor
					if(isset($snemp->supSupervisor) && !empty($snemp->supSupervisor) && $snemp->supSupervisor!=0) $this->staffM->addMyNotif($snemp->supSupervisor, $snote, 0, 1, 0); //to 2nd level manager
				}
				
				//send notification to HR if recommendation is follow up another coaching
				if($_POST['recommendation']==4 || $_POST['recommendation']==6){					
					$hrnote = 'Recommendation for coaching evaluation is "'.$recText.'". Please print coaching form. Click <a href="'.$this->config->base_url().'coachingform/evaluation/'.$id.'/" class="iframe">here</a> to view and download the file.';
					$hrStaffs = $this->staffM->getHRStaffID();
					for($hr=0; $hr<count($hrStaffs); $hr++){
						if($hrStaffs[$hr] != $this->user->empID)
					...
					//create CIS
					if($_POST['recommendation']==6){
						$sinfo = $this->staffM->getSingleInfo('staffCoaching', 'staffCoaching.empID_fk, staffs.position, title, supervisor, (SELECT CONCAT(fname," ",lname) AS n FROM staffs s WHERE s.empID=staffs.empID) AS supName', 'coachID="'.$id.'"', 'LEFT JOIN staffs ON empID=empID_fk LEFT JOIN newPositions ON posID=position');
						
						$ss = explode('|', $_POST['transferData']);
						$newposname = $this->staffM->getSingleField('newPositions', 'title', 'posID="'.$ss[0].'"');
						$newsupname = $this->staffM->getSingleField('staffs', 'CONCAT(fname," ",lname) AS n', 'empID="'.$ss[1].'"');
																	
						$updateArr['position'] = $ss[0];
						$updatetext['position'] = array(
											'c' => $sinfo->title,
											'n' => $newposname
										);	
						
						$updateArr['supervisor'] = $ss[1];
						$updatetext['supervisor'] = array(
											'c' => $sinfo->supName,
											'n' => $newsupname
										);
						...
						$hrnote2 = 'CIS has been created for the coaching evaluation. Click <a href="'.$this->config->base_url().'cispdf/'.$insid.'/">here</a> to view and print CIS form.';
						for($hr=0; $hr<count($hrStaffs); $hr++){
							if($hrStaffs[$hr] != $this->user->empID)
								$this->staffM->addMyNotif($hrStaffs[$hr], $hrnote2, 0, 1, 0);
						}
					}
				}
	...
	public function staffcoaching(){
		...
		if($this->user!=false){		
			if($this->user->access=='' && $this->user->level==0){
				...
				$data['pending'] = $this->staffM->getQueryResults('staffCoaching', 'staffCoaching.*, CONCAT(fname," ",lname) AS name, username, (SELECT CONCAT(fname," ",lname) AS n FROM staffs s WHERE s.empID=staffs.supervisor) AS coachedBy', 'status=0 AND coachedEval<="'.date('Y-m-d').'"'.$condition, 'LEFT JOIN staffs ON empID=empID_fk');
